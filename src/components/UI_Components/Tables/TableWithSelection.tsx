...
export const TableWithSelection = (props: TableWithSelectionProps) => {
    ...
    const [searchTerm, setSearchTerm] = React.useState<string>(headCells[0].id)
    ...
    let handleSearchTermChange = (val: string) => {
        console.log(val)
        setSearchTerm(val)
    }

    let searchFn = (key: string) => {
            ...
            <Paper sx={{width: '100%', mb: 2}}>
              ...
              rows.slice().sort(getComparator(order, orderBy)) */}
                            {stableSort(rows, getComparator(order, orderBy))
                                ...
                                .map((row, index) => {
                                    let mainKey: any = row[rowMainKey];
